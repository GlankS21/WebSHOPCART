            ...
            <div class="product-list">
                ...
                if (isset($_GET['sort'])) {
                    ...
                    switch ($sort) {
                        ...
                        case 'popular':
                        default:
                            $orderBy = "ORDER BY shoes_id DESC"; 
                            break;
                    }
                } else {
                    $orderBy = "ORDER BY shoes_id DESC";
                }
                $sql = "SELECT shoes_id, name, price, image FROM shoes " . $orderBy;
                $stmt = $conn->query($sql); 
                if ($stmt->rowCount() > 0) {
                    while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo '<div class="product-item" onclick="window.location.href=\'productcard.php?id=' . $row["shoes_id"] . '&category=shoes\'">';
